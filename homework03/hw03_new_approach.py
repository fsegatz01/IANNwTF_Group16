...
def prepare_mnist_data(mnist):
    # mnist = mnist.map(lambda img, target: (tf.reshape)))
    ...
    mnist = mnist.map(lambda img, target: ((img / 128.) - 1., target))  # adjust values from ... to ... ?
...
train_losses0, test_losses0, test_accuracies0, train_accuracies0 = training_loop(num_epochs, model, train_dataset,
                                                                                 test_dataset,
...
vizualize(train_losses0, test_losses0, test_accuracies0, train_accuracies0, "Original") #changed learning rate to 0.2